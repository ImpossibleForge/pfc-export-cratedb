#!/usr/bin/env python3
"""
pfc-export-cratedb — Export CrateDB tables to PFC cold-storage archives.

Streams rows from a CrateDB table directly into a compressed .pfc archive
with block-level timestamp index — ready for time-range queries via DuckDB
or pfc-gateway without loading the full archive.

Usage:
  pfc-export-cratedb --host crate.example.com --table application_logs --output logs.pfc
  pfc-export-cratedb --host crate.example.com --table events \\
    --ts-column timestamp \\
...
def export_to_pfc(
    ...
    """
    Stream rows from CrateDB into a PFC archive.

    Flow: CrateDB → fetchmany() loop → JSONL temp file → pfc_jsonl compress → .pfc + .pfc.bidx
    ...
    if verbose:
        print(f"  → Connecting to CrateDB at {host}:{port} (db: {dbname}) ...")
    ...
    tmp_fd, tmp_jsonl = tempfile.mkstemp(suffix=".jsonl", prefix="pfc_cratedb_")
    os.close(tmp_fd)
...
def build_parser():
    parser = argparse.ArgumentParser(
        prog="pfc-export-cratedb",
        description="Export CrateDB tables to PFC cold-storage archives.",
...
Install pfc_jsonl binary first:
  curl -L https://github.com/ImpossibleForge/pfc-jsonl/releases/latest/download/pfc_jsonl-linux-x64 \\
    ...
    parser.add_argument("--version", action="version", version=f"pfc-export-cratedb {__version__}")
    parser.add_argument("--host",       required=True, help="CrateDB hostname or IP")
    parser.add_argument("--port",       type=int, default=5432, help="PostgreSQL port (default: 5432)")
    parser.add_argument("--user",       default="crate",        help="Username (default: crate)")
    parser.add_argument("--password",   default="",             help="Password (default: empty)")
    parser.add_argument("--dbname",     default="doc",          help="Database name (default: doc)")
    parser.add_argument("--schema",     default="doc",          help="Schema (default: doc)")
...
def main():
    ...
    try:
        stats = export_to_pfc(
            ...
            password=args.password, dbname=args.dbname, schema=args.schema,
